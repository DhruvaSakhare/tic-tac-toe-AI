...
def minimax(b, depth, isMaximising):
    r = gameover(b)
    if r == 1:
        return 1, None
    elif r == 2:
    ...
    if isMaximising:
        ...
        for (i, j) in pm:
            if b[i][j] == 0:
                nb = copy.deepcopy(b)
                nb[i][j] = 1
                currentScore, bestMove = minimax(nb, depth+1, False)
        ...
        for (i, j) in pm:
            if b[i][j] == 0:
                nb = copy.deepcopy(b)
                nb[i][j] = 2
                currentScore, bestMove = minimax(nb, depth+1, True)
                if score > currentScore:
                    score = currentScore
                    move = (i, j)
        return score, move
